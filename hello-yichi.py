...
NUM = 20
dataPredImi = np.zeros((NUM, 39, 482, 1), dtype='float64')

y, fs = librosa.load('./public/uploads/demo.wav', sr=16000)
if y.shape[0] < 4 * fs:
    temp1 = np.zeros((4 * fs - y.shape[0]))
    y_fix = np.concatenate((y, temp1), axis=0)
else:
    temp2 = y[0:4 * fs]
    y_fix = temp2

S = librosa.feature.melspectrogram(y=y_fix, sr=fs, n_fft=133, hop_length=133, power=2, n_mels=39, fmin=0.0, fmax=5000)
S = S[:, 0:482]
S_db = librosa.power_to_db(S, ref=np.max)
for j in range(NUM):
    dataPredImi[j, :, :, 0] = S_db

dataPredImi = dataPredImi.astype('float64')
...
classesPred = labelPredRec.shape[0]
...
retr_pairs_left = dataPredImi
retr_pairs_right = dataPredRec

retr_pairs_left = retr_pairs_left.reshape(retr_pairs_left.shape[0], 1, 39, 482)
retr_pairs_right = retr_pairs_right.reshape(retr_pairs_right.shape[0], 1, 128, 128)

get_model()

pred3 = overall_model.predict([retr_pairs_left, retr_pairs_right])
n = 5
ids = np.squeeze(pred3).argsort()[::-1][:n]

temp0 = 100.1 # ids[0]

FILENAMES = [
'marimba_hit_with_a_rubber_mallet',
'oboe',
'orchestra_bells',
'piano',
'tambourine',
'tambourine_shake_roll',
'thaigong',
'triangle',
'trombone',
'trumpet',
...
string = '\n' + '(1) ' + '<a href=\"' + './log_mel_44.1k_data/second_half_audio/ai/' + FILENAMES[ids[0]] + '.wav' + '\">' + FILENAMES[ids[0]] + '</a>'+ \
'\n' + '(2) ' + '<a href=\"' + './log_mel_44.1k_data/second_half_audio/ai/' + FILENAMES[ids[1]] + '.wav' + '\">' + FILENAMES[ids[1]] + '</a>'+ \
'\n' + '(3) ' + '<a href=\"' + './log_mel_44.1k_data/second_half_audio/ai/' + FILENAMES[ids[2]] + '.wav' + '\">' + FILENAMES[ids[2]] + '</a>'+ \
'\n' + '(4) ' + '<a href=\"' + './log_mel_44.1k_data/second_half_audio/ai/' + FILENAMES[ids[3]] + '.wav' + '\">' + FILENAMES[ids[3]] + '</a>'+ \
'\n' + '(5) ' + '<a href=\"' + './log_mel_44.1k_data/second_half_audio/ai/' + FILENAMES[ids[4]] + '.wav' + '\">' + FILENAMES[ids[4]] + '</a>'
